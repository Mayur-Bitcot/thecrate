...
(function ($) {
  ...
  // Buttons
  jQuery(".themeslr_button_shortcode a")
    .mouseover(function () {
      // bg
    ...
    var TheCrateQtyPlusMinus = {
      init: function () {
        ...
        if (singleProductQtyBox.length) {
          $(
            "#woosq-popup form.cart, form.cart, form.woocommerce-cart-form, .woobt-products"
      ...
      init: function () {
        ...
        if ($rowOverlays.length) {
          $rowOverlays.each(function (i) {
            var thisItem = $(this),
              thisItemParent = $(this).parent().parent().parent().parent();

            thisItem.prependTo(thisItemParent);
          });
        }
      },
    };
  ...
  jQuery(document).ready(function () {
    if (
      jQuery(".woocommerce div.product form.cart .variations select").length ==
        0 ||
      jQuery(".widget_archive select").length == 0 ||
      jQuery(".widget_categories select").length == 0 ||
      jQuery(".widget_text select").length == 0 ||
      jQuery(".wpcf7-select").length == 0
    ) {
      jQuery(
        ".woocommerce div.product form.cart .variations select, .widget_archive select, .widget_categories select, .widget_text select, .wpcf7-select"
      ).niceSelect();
    }
    ...
    if (jQuery(window).width() < 768) {
      ...
      jQuery(
        ".navbar-collapse .menu-item-has-children, .navbar-collapse .mega1column"
      ).append(expand);
    ...
    jQuery(window).scroll(function () {
      jQuery(this).scrollTop() > offset
        ? $back_to_top.addClass("themeslr-is-visible")
        : $back_to_top.removeClass("themeslr-is-visible themeslr-fade-out");
